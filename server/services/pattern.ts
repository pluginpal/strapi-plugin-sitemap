...
const getAllowedFields = (contentType, allowedFields = []) => {
  ...
  fieldTypes.map((fieldType) => {
    Object.entries(contentType.attributes).map(([fieldName, field]: any) => {
      if ((field.type === fieldType || fieldName === fieldType) && field.type !== 'relation') {
        ...
        Object.entries(relation.attributes).map(([subFieldName, subField]: any) => {
 ...
 */
const validatePattern = (pattern, allowedFieldNames) => {
  if (!pattern) {
    return {
      valid: false,
